    ...
    suite.addTest("Form validation works", async () => {
      ...
      try {
        ...
        const isValidCorrect =
          window.ValidationService.validateEmail("test@example.com");
        if (!isValidCorrect) {
      ...
      for (const form of forms) {
        ...
        for (const input of inputs) {
          if (!input.id && !input.getAttribute("aria-label")) {
            console.warn(
              `Form input without ID or aria-label: ${input.tagName}`,
            );
          }
        }
      }
  ...
  static run(suite) {
    ...
    suite.addTest("Lazy loading is implemented", async () => {
      if (typeof IntersectionObserver === "undefined") {
        console.warn(
          "IntersectionObserver not supported - lazy loading may not work",
        );
      }
    });
