...
class OmniKrossApp {
  ...
  async init() {
    ...
    try {
      ...
      console.log(`OmniKross v3.0 initialized: ${this.state.lang}/${this.state.role}`)
    } catch (error) {
  ...
  addLiveValidation(form) {
    ...
    if (socialInput) {
      socialInput.addEventListener('blur', () => this.validateField(socialInput, 'social'))
  ...
  validateField(input, fieldType) {
    ...
    switch (fieldType) {
      case 'email':
        ...
        errorMessage = this.state.lang === 'ru' ? 'Введите корректный email' : 'Enter a valid email'
        break
      ...
      case 'social': {
        ...
        errorMessage = this.state.lang === 'ru' ? 'Формат: @username' : 'Format: @handle'
        break
      }
  ...
  async handleFormSubmit(e) {
    ...
    // Validate form
    const emailValid = this.validateField(form.querySelector('#email'), 'email')
    const socialValid = this.validateField(form.querySelector('#social'), 'social')

    if (!emailValid || !socialValid) return

    this.state.isSubmitting = true
    const submitButton =
      form.querySelector('button[type="submit"]') || document.getElementById('ctaButton')

    if (submitButton) {
      submitButton.disabled = true
      submitButton.textContent = this.state.lang === 'ru' ? 'Отправка...' : 'Submitting...'
    }
    ...
    try {
      ...
      const result = await this.mockApiCall('/api/signup', {
        ...
        simulateResponse: {
          success: true,
          message:
            this.state.lang === 'ru' ? 'Успешно зарегистрированы!' : 'Successfully registered!',
  ...
  async handleSuccessfulSubmission(result, submitButton) {
    ...
    if (submitButton) {
      ...
      submitButton.innerHTML = `<span>✓ ${
        result.message ||
        (this.state.lang === 'ru' ? 'Вы пионер #' : "You're Pioneer #") + result.slotNumber + '!'
      }</span>`
    }
  ...
  handleSubmissionError(error, submitButton) {
    if (submitButton) {
      ...
      submitButton.textContent =
        this.state.lang === 'ru' ? 'Ошибка, попробуйте снова' : 'Error, try again'
...
class ROICalculator {
  ...
  calculate() {
    ...
    const rate =
      window.OmniKrossApp?.config?.rates?.[document.documentElement.lang || 'ru']?.hour || 25
    ...
    if (moneyEl) {
      moneyEl.textContent =
        ...
        ' ' +
        (window.OmniKrossApp?.config?.rates?.[document.documentElement.lang || 'ru']?.currency ||
          '$')
    }
